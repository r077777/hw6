...
window.addEventListener('DOMContentLoaded', async function(event) {
  ...
  let moviePicking = document.querySelector('.movies')
  
  //adds the firebase data
  let db = firebase.firestore()
  let querySnapshot = await db.collection('movies').get()
  let watchedMovies = querySnapshot.docs

  
  //inserts movies onto the page
  for (let i = 0; i<json.results.length; i++) {
    moviePicking.insertAdjacentHTML('beforeend', `
    ...
    //sets initial opacity from persisted firebase data
    for (let f=0; f<watchedMovies.length; f++) {
      let watched = watchedMovies[f].data() 
      if (json.results[i].id == watched.movieID) {
  ...
  //https://api.themoviedb.org/3/movie/now_playing?2abc36961f04ade33bace94a9b8ea083language=en-US
